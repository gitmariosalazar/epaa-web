...
import { useUpdateSpecialReading } from '../hooks/useUpdateSpecialReading';
...
import { ReadingSpecialUpdateInfoForm } from '../components/ReadingSpecialUpdateInfoForm';
...
export const UpdateSpecialReadingPage: React.FC<UpdateReadingPageProps> = ({
  ...
  const {
    ...
    error,
    fetchReadingData,
    submitUpdateSpecialReading,
    clearData
  } = useUpdateSpecialReading();
  ...
  useEffect(() => {
    if (readingInfo && readingInfo.length > 0) {
      ...
      if (info.previousReading !== null && info.previousReading !== undefined) {
        setPreviousReadingInput(info.previousReading);
      } else {
  ...
  const buildRequest = (): UpdateSpecialReadingRequest => ({
    tipoAjusteId: Number(tipoAjusteId),
    justificacion: observationInput,
    previousReading: previousReadingInput === '' ? null : Number(previousReadingInput),
  ...
  const handleSave = () => {
    ...
    if (observationInput.trim() === '') {
      alert('Por favor, ingrese una justificación para el ajuste especial.');
      return;
    }
  ...
  const handleConfirm = async () => {
    ...
    const result = await submitUpdateSpecialReading(currentReadingId, requestPayload);
    ...
    <div className="cr-container">
      <div className="cr-content-wrapper">
        <div className="cr-header-container">
          <h2 className="cr-page-title">Actualización y Corrección de Lecturas</h2>
