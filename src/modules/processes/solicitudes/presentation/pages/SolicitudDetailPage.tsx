...
const PhaseActionBtn: React.FC<ActionBtnProps> = ({
  color,
  bg,
  icon,
  label,
  ...
  >
    <div className="sol-phase-action-btn__content">
      <div className="sol-phase-action-btn__icon">
        {loading ? (
          <Clock size={18} className="sol-detail-loading__spinner" />
        ) : (
          icon
        )}
      </div>
      <div className="sol-phase-action-btn__main">
        <div className="sol-phase-action-btn__info">
          <span className="sol-phase-action-btn__label">
            {loading ? 'Procesando...' : label}
          </span>
          {description && (
            <span className="sol-phase-action-btn__description">
              {description}
            </span>
          )}
        </div>
...
export const SolicitudDetailPage: React.FC = () => {
  ...
  const [assignAnalystOpen, setAssignAnalystOpen] = useState(false);
  ...
  const [submitCorrectionsModalOpen, setSubmitCorrectionsModalOpen] =
    useState(false);
  ...
  const requestDetailUseCase = React.useMemo(
    () => new GetRequestDetailByRequestIdOrNumberUseCase(repo),
    [repo]
  );
  const trackingUseCase = React.useMemo(
    () => new GetTrackingBySolicitudIdUseCase(repo),
    [repo]
  );
  const workOrderUseCase = React.useMemo(
    () => new GetOrdenesTrabajoBysSolicitudIdUseCase(repo),
    [repo]
  );
  ...
  const rejectPaymentUseCase = React.useMemo(
    () => new RejectPaymentUseCase(repo),
    [repo]
  );
  const startInspectionUseCase = React.useMemo(
    () => new StartInspectionUseCase(repo),
    [repo]
  );
  const startInstallUseCase = React.useMemo(
    () => new StartInstallationUseCase(repo),
    [repo]
  );
  const updateDocUseCase = React.useMemo(
    () => new UpdateConnectionDocumentUseCase(repo),
    [repo]
  );
  const uploadReceiptUseCase = React.useMemo(
    () => new UploadInspectionInvoiceReceiptUseCase(repo),
    [repo]
  );
  ...
  const handleConfirmPayment = async (localFile?: File) => {
    if (!solicitud?.facturaId) {
      MessageToastCustom('error', 'Error', 'ID de factura no encontrado.');
      return;
    }
    if (!paymentReference.trim()) {
      MessageToastCustom(
        'error',
        'Requerido',
        'Ingrese la referencia de pago.'
      );
      return;
    }
    if (!user?.userId) {
      MessageToastCustom('error', 'Sesión', 'Inicie sesión nuevamente.');
      return;
    }
    ...
    try {
      ...
      if (localFile) {
        const uploadSuccess = await uploadReceiptUseCase.execute(
          solicitud.facturaId,
          localFile
        );
        if (!uploadSuccess) {
          throw new Error(
            'No se pudo subir el comprobante. Abortando confirmación.'
          );
        }
      }
      ...
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'Error al confirmar el pago.'
      );
    } finally {
  ...
  const handleRejectPayment = async (reason: string) => {
    ...
    try {
      ...
      MessageToastCustom(
        ...
        'El trámite regresó a Recolección de Pago.'
      );
      ...
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'Error al rechazar el pago.'
      );
    } finally {
  ...
  const handleDocFileReplace = async (
    docId: string,
    file: File,
    ...
    try {
      ...
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'Error al actualizar el documento.'
      );
    } finally {
  ...
  const handleBulkCorrectionsSubmit = async (
    files: File[],
    documentIds: string[]
  ) => {
    if (!solicitud || !user) return;
    ...
    try {
      ...
      if (allSuccess) {
        MessageToastCustom(
          'success',
          'Correcciones Subidas',
          'Los documentos fueron subidos y enviados a revisión correctamente.'
        );
      ...
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'Error al subir las correcciones en lote.'
      );
    } finally {
  ...
  const handleStartInspection = async () => {
    ...
    const otInspeccion = workOrders.find((wo) => wo.tipoOrden === 'INSPECCION');
    const workOrderId = otInspeccion?.workOrderId ?? '';
    if (!workOrderId) {
      MessageToastCustom(
        'error',
        'Error',
        'No se encontró el ID de la OT de inspección. Recarga la página.'
      );
      return;
    }
    if (!user?.userId) {
    ...
    try {
      await startInspectionUseCase.execute({
        workOrderId,
      ...
      MessageToastCustom(
        'success',
        'Inspección Iniciada',
        'El estado cambió a INSPECCION_EN_PROCESO.'
      );
      reload();
    } catch (e: any) {
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'No se pudo iniciar la inspección.'
      );
    } finally {
  ...
  const handleStartInstallation = async () => {
    ...
    if (!workOrderId) {
      MessageToastCustom(
        'error',
        'Error',
        'No se encontró el ID de la OT de instalación. Recarga la página.'
      );
      return;
    }
    if (!user?.userId) {
    ...
    try {
      await startInstallUseCase.execute({
        workOrderId,
        technicianId: user.userId
      });
      MessageToastCustom(
        ...
        'El estado cambió a INSTALACION_EN_PROCESO.'
      );
      reload();
    } catch (e: any) {
      MessageToastCustom(
        'error',
        'Error',
        e.message || 'No se pudo iniciar la instalación.'
      );
    } finally {
    ...
    >
      <div className="sol-detail-container">
        ...
        <div className="sol-detail-main-col">
          ...
          <SolicitudHeroCard solicitud={solicitud} />

          {/* ══ ACCIONES POR FASE ══ */}
          <div className="sol-detail-phase-actions">
            {/* Fase 3: Validar documentos */}
            {solicitud.estado === 'DOCS_SUBMITTED' &&
              solicitud.analistaUsername === null && (
                <PhaseActionBtn
                  color="#f59e0b"
                  bg="rgba(245,158,11,0.1)"
                  icon={<FileCheck size={18} />}
                  label="Asignar Analista"
                  description="Asigna un analista para revisar los documentos presentados por el cliente"
                  onClick={() => setAssignAnalystOpen(true)}
                />
              )}
            {/* Fase 3: Validar documentos */}
            {solicitud.estado === 'DOCS_SUBMITTED' &&
            ...
            {solicitud.estado === 'INSPECCION_EN_PROCESO' &&
              !solicitud.informeId && (
                <PhaseActionBtn
                  ...
                  disabled={
                    workOrders.find(
                      (o) => o.codigoOrden === solicitud.solicitudNumero
                    )?.estadoOt !== 'INSPECCION_COMPLETADA'
                  }
      ...
      {/* Asignar analista */}
      {assignAnalystOpen && (
        <AssignAnalystToSolicitudModal
          isOpen={assignAnalystOpen}
          onClose={() => setAssignAnalystOpen(false)}
          solicitud={solicitud}
          onSubmit={() => {
            setAssignAnalystOpen(false);
            reload();
          }}
        />
      )}
      ...
      <PaymentReceiptPreviewModal
        ...
        handleConfirmPayment={async (file) => {
          await handleConfirmPayment(file);
      ...
      {solicitud.documentos && (
        <SubmitCorrectionsModal
          ...
          rejectedDocuments={solicitud.documentos.filter(
            (d) =>
              d.estadoValidacion === 'RECHAZADO' ||
              d.estadoValidacion === 'INVALIDO'
          )}
      ...
      {submitReportOpen && (
        <SubmitInspectionReportModal
          ...
          workOrderId={
            workOrders.find(
              (wo) =>
                (solicitud && wo.codigoOrden === solicitud.solicitudNumero) ||
                wo.tipoOrden === 'INSPECCION'
            )?.workOrderId ?? ''
          }
          codigoOrden={
            workOrders.find(
