...
import { FileText, X, DollarSign, MapPin, Clock, CheckCircle } from 'lucide-react';
...
const useCase = new SubmitInspectionReportUseCase(new SolicitudRepositoryImpl());

const RESULTS = [
  { value: 'VIABLE',     label: 'Viable — Se puede realizar la acometida' },
  { value: 'NO_VIABLE',  label: 'No Viable — No es posible realizar la acometida' },
  { value: 'CONDICIONAL',label: 'Condicional — Viable con condiciones especiales' },
];

export const SubmitInspectionReportModal: React.FC<SubmitInspectionReportModalProps> = ({
  isOpen, onClose, solicitudId, solicitudNumero, workOrderId, technicianId, onSuccess
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!workOrderId) {
      MessageToastCustom('error', 'Error', 'No se encontró el ID de la orden de trabajo. Recargue la página.');
      return;
    }
    setLoading(true);
    try {
      await useCase.execute({
        ...
        solicitudId,
        result,
        networkDistanceM: networkDistanceM ? Number(networkDistanceM) : undefined,
      ...
      MessageToastCustom('success', 'Informe Enviado', 'El informe técnico fue subido y está en revisión.');
      onSuccess();
      onClose();
    } catch (err: any) {
      MessageToastCustom('error', 'Error', err.message || 'No se pudo enviar el informe.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="action-modal-overlay" onClick={onClose}>
      <div className="action-modal action-modal--wide" onClick={e => e.stopPropagation()}>
        <div className="action-modal__header action-modal__header--purple">
          <div className="action-modal__header-icon"><FileText size={20} /></div>
          <div>
            <h3 className="action-modal__title">Enviar Informe Técnico</h3>
            <p className="action-modal__subtitle">Solicitud {solicitudNumero} · Fase 8</p>
          </div>
        ...
        <form className="action-modal__body" onSubmit={handleSubmit}>
          <div className="action-modal__section-title">Resultado de la Inspección</div>
          <div className="action-modal__field">
            <label className="action-modal__label"><CheckCircle size={13} /> Dictamen Técnico</label>
            <div className="action-modal__radio-group">
              {RESULTS.map(r => (
                <label key={r.value} className={`action-modal__radio${result === r.value ? ' action-modal__radio--selected' : ''}`}>
                  <input type="radio" name="result" value={r.value} checked={result === r.value} onChange={e => setResult(e.target.value)} />
                  <span>{r.label}</span>
                </label>
              ))}
            </div>
          ...
          <div className="action-modal__row">
            <div className="action-modal__field">
              <label className="action-modal__label"><MapPin size={13} /> Distancia a la Red (m)</label>
              <input
                type="number"
                min="0"
                step="0.1"
          ...
          <div className="action-modal__row">
            <div className="action-modal__field">
              <label className="action-modal__label"><DollarSign size={13} /> Costo Materiales ($)</label>
              <input
                type="number"
                min="0"
                step="0.01"
            ...
            <div className="action-modal__field">
              <label className="action-modal__label"><DollarSign size={13} /> Costo Mano de Obra ($)</label>
              <input
                type="number"
                min="0"
                step="0.01"
          ...
          <div className="action-modal__field">
            <label className="action-modal__label">Condiciones del Terreno</label>
            <input
              type="text"
              className="action-modal__input"
          ...
          <div className="action-modal__field">
            <label className="action-modal__label"><FileText size={13} /> Observaciones del Campo</label>
            <textarea
          ...
          <div className="action-modal__actions">
            ...
            <Button
              ...
              type="submit"
              disabled={loading}
              leftIcon={loading ? <Clock size={15} className="spin-icon" /> : <CheckCircle size={15} />}
            >
