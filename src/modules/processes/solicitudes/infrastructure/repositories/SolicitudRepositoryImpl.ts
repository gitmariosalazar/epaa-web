...
export class SolicitudRepositoryImpl implements SolicitudRepository {
  ...
  async getOrdenesTrabajoBysSolicitudId(
    ...
    // Ajusta el endpoint según tu backend real. Ejemplo:
    const response = await this.client.get<
  ...
  async createInspectionInvoice(
    dto: CreateInspectionInvoiceDto
  ): Promise<void> {
    ...
    await this.client.post<void>(
      '/inspection-invoice/create_inspection_invoice',
      dto
    );
  }
  ...
  async submitInspectionReport(dto: SubmitInspectionReportDto): Promise<void> {
    ...
    if (!dto.solicitudId) throw new Error('El ID de la solicitud es requerido');
    if (!dto.result)
      throw new Error('El resultado de la inspección es requerido');
    if (!dto.technicianId) throw new Error('El ID del técnico es requerido');
  ...
  async signContract(dto: SignContractDto): Promise<void> {
    ...
    if (!dto.signatureStatus)
      throw new Error('El estado de firma es requerido');
    if (!dto.signedContractUrl)
      throw new Error('La URL del contrato firmado es requerida');
  ...
  async registerCadastral(dto: RegisterCadastralDto): Promise<void> {
    ...
    if (!dto.registratorId)
      throw new Error('El ID del registrador es requerido');

    await this.client.post<void>(
      '/cadastral/solicitudes/registro-catastral',
      dto
    );
  }
  ...
  async submitWithDocuments(
    ...
    if (!dto.userId) throw new Error('El ID del usuario es requerido');
    if (!dto.clientId) throw new Error('El ID del cliente es requerido');
    if (!dto.personType) throw new Error('El tipo de persona es requerido');
    if (!dto.connectionType)
      throw new Error('El tipo de conexión es requerido');
    if (!dto.propertyUse)
      throw new Error('El uso de la propiedad es requerido');
    if (!dto.address) throw new Error('La dirección es requerida');
    if (dto.longitude === undefined)
      throw new Error('La longitud es requerida');
    if (dto.latitude === undefined) throw new Error('La latitud es requerida');
    if (!dto.additionalInfo)
      throw new Error('La información adicional es requerida');
    if (!dto.documents) throw new Error('Los documentos son requeridos');

    const formData = new FormData();
    formData.append('clientId', dto.clientId);
    formData.append('userId', dto.userId);
    formData.append('personType', dto.personType);
    formData.append('connectionType', dto.connectionType);
    formData.append('propertyUse', dto.propertyUse);
    formData.append('address', dto.address);
    formData.append('cadastralKey', dto.cadastralKey);
    ...
    formData.append('additionalInfo', JSON.stringify(dto.additionalInfo));

    const docTypeIds: string[] = [];
    dto.documents.forEach((doc) => {
      docTypeIds.push(doc.documentTypeId);
      if (doc.file) {
    ...
    // Print the DTO being sent
    const debugDto: Record<string, unknown> = {};
    Array.from(formData.keys()).forEach((key) => {
      const values = formData.getAll(key);
      if (values.length === 1) {
        const val = values[0];
        if (val instanceof File) {
          debugDto[key] = { name: val.name, size: val.size, type: val.type };
        } else {
          try {
            debugDto[key] = JSON.parse(val as string);
          } catch {
            debugDto[key] = val;
          }
        }
      } else {
        debugDto[key] = values.map((val) => {
          if (val instanceof File) {
            return { name: val.name, size: val.size, type: val.type };
          }
          try {
    ...
    >('/requests/submit-with-documents', formData, {
      headers: {
