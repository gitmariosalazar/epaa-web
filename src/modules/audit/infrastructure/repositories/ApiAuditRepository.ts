...
export class ApiAuditRepository implements AuditRepository {
  ...
  async getAuditLogs(
    ...
    const response = await this.client.get<ApiResponse<AuditRegistroResponse[]>>(
      '/audit-gateway/get-logs',
      { params: apiParams }
    );
  ...
  async getSessionLogs(
  ...
  ): Promise<ApiResponse<AuditSessionResponse[]>> {
    // Map camelCase frontend params to snake_case backend params
    const apiParams = {
      limit: params.limit,
      offset: params.offset,
    ...
    const response = await this.client.get<ApiResponse<AuditSessionResponse[]>>(
      '/audit-gateway/get-session-logs',
      { params: apiParams }
    );
    return response.data;
  }
